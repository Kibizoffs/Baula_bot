...
def pat_frames(ava):
    ...
    for i in range(c):
        ...
        pat_coords = (x - squish_x // 2 + shake_x, y - squish_y + shake_y)
        frame.paste(
...
async def command_hand(msg: Message):
    if len(msg.text.split()) > 1:
        ...
        try:
            user = await bot.get_chat_member(msg.chat.id, user_id=user_id)
            user = user.user
        except:
...
async def command_stick(msg: Message):
    stick = random.choice([stick_false, '0' + '=' * random.randint(5,20) + 'D'])
...
async def command_sal(msg: Message):
    ...
    user2_id = args[2]
    try:
        user1 = await bot.get_chat_member(msg.chat.id, user_id=user1_id)
        user2 = await bot.get_chat_member(msg.chat.id, user_id=user2_id)
        user1 = user1.user
        user2 = user2.user
    except:
        await msg.answer(sal_2_arguments)
